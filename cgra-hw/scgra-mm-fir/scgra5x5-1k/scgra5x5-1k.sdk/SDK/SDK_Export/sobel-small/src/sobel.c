...
int Verification(){
	...
	unsigned char* fPtr = (unsigned char*)FPGA_Fig_Out;
	for(i=0; i<R; i++){
		for(j=0; j<C; j++){
			if(ARM_Fig_Out[i][j]!=fPtr[i*C+j]){
				printf("Verification failed! ARM_Fig_Out[%d][%d]=%d, FPGA_Fig_Out[%d][%d]=%d\n\r", i, j, ARM_Fig_Out[i][j], i, j, fPtr[i*C+j]);
				return XST_FAILURE;
			}
		}
	}
...
int FPGA_Acc(void){
	...
	XDma_PS_Move((u32)(&Fig_In[0]), Input_Buffer_Addr, R*C/4, 0);

	Input_Buffer_Addr = Bram0_Base_Addr + (Const_Num + R*C/4)*4;
	XDma_PS_Move((u32)(&Gx[0][0]), Input_Buffer_Addr, 3*3, 0);

	Input_Buffer_Addr = Bram0_Base_Addr + (Const_Num + R*C/4 + 3*3)*4;
	...
	Xil_DCacheFlushRange((u32)(FPGA_Fig_Out), R*C);
	Output_Buffer_Addr = Bram1_Base_Addr;
	XDma_PS_Move(Output_Buffer_Addr, (u32)(&FPGA_Fig_Out[0][0]), R*C/4, 0);

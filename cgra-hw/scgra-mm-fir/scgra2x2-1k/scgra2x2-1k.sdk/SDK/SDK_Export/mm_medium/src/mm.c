...
void FPGA_Kernel(u32 Block_A_Addr, u32 Block_B_Addr, u32 Block_C_Addr, int Print_En){
	...
	XDma_PS_Move(Block_A_Addr, Input_Buffer_Addr, B*N, 0);

	Input_Buffer_Addr = Bram0_Base_Addr + (Const_Num + B*N)*4;
	XDma_PS_Move(Block_B_Addr, Input_Buffer_Addr, N, 0);
	...
	Xil_DCacheFlushRange(Block_C_Addr, B*4);
	Output_Buffer_Addr = Bram1_Base_Addr;
	XDma_PS_Move(Output_Buffer_Addr, Block_C_Addr, B, 0);
	if(Print_En){
...
int FPGA_Acc(void){
	...
	for(i=0; i<N; i=i+B){
		for(j=0; j<N; j++){
			...
			for(k=0; k<B; k++){
				FPGA_C_Out[i+k][j] = Block_C_Out[k];
			}

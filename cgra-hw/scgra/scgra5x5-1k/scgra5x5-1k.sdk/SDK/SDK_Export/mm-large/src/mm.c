...
void FPGA_Kernel(u32 Block_A_Addr, u32 Block_B_Addr, u32 Block_C_Addr, int Print_En);
...
void FPGA_Kernel(u32 Block_A_Addr, u32 Block_B_Addr, u32 Block_C_Addr, int Print_En){
	...
	XDma_PS_Move(Block_A_Addr, Input_Buffer_Addr, B, 0);

	Input_Buffer_Addr = Bram0_Base_Addr + (Const_Num + B)*4;
	XDma_PS_Move(Block_B_Addr, Input_Buffer_Addr, B, 0);
	...
	Xil_DCacheFlushRange(Block_C_Addr, (B/M)*4);
	Output_Buffer_Addr = Bram1_Base_Addr;
	//XDma_PS_Move(Output_Buffer_Addr, Block_C_Addr, B/M, 0);
	GPIO_Move(Block_C_Out,Output_Buffer_Addr, B/M, 0);
	if(Print_En){
...
int FPGA_Acc(void){
	...
	for(i=0; i<N; i++){
		for(j=0; j<N; j++){
			...
			PS_Timer_Start();
			// Move a row of A_In to FPGA
			// This can be optimized as it can be reused for different iterations.
			Block_A_Addr = (u32)(&A_In[i][0]);

			// Move a column to Block_B_In
			for(k=0; k<N; k++){
				Block_B_In[k] = B_In[k][j];
			}

			Block_B_Addr = (u32)(&Block_B_In[0]);
			Block_C_Addr = (u32)(&Block_C_Out[0]);
			if(i==0 && j==0){
			...
			FPGA_Kernel(Block_A_Addr, Block_B_Addr, Block_C_Addr, Print_En);

			PS_Timer_Start();
			// Move result back to FPGA_C_Out
			FPGA_C_Out[i][j] = 0;
			for(k=0; k<B/M; k++){
				FPGA_C_Out[i][j] += Block_C_Out[k];
			}
...
void CPU_Compute(){
	...
	for(i=0; i<N; i++){
		for(j=0; j<N; j++){
			ARM_C_Out[i][j] = 0;
			for(k=0; k<N; k++){
				ARM_C_Out[i][j] += A_In[i][k] * B_In[k][j];
			}
		}
	}

...
void FPGA_Kernel(u32 Block_In_Addr, u32 Block_Out_Addr, int Print_En);


int main() {

	printf("--------------------------------------------------------------------------------\n\r");
	printf("Fir (N=%d, L=%d) computation using CGRA!\n\r", N, L);
	printf("--------------------------------------------------------------------------------\n\r");
...
int Verification(){
	...
	for(i=0; i<L; i++){
		...
		if(ARM_Sig_Out[i]!=FPGA_Sig_Out[i]){
			printf("Verification failed! ARM_Sig_Out[%d]=%d, FPGA_Sig_Out[%d]=%d\n\r", i, ARM_Sig_Out[i], i, FPGA_Sig_Out[i]);
			return XST_FAILURE;
		}
...
void FPGA_Kernel(u32 Block_In_Addr, u32 Block_Out_Addr, int Print_En){
	...
	XDma_PS_Move(Block_In_Addr, Input_Buffer_Addr, N+B, 0);
	...
	Xil_DCacheFlushRange(Block_Out_Addr, B*4);
	Output_Buffer_Addr = Bram1_Base_Addr;
	XDma_PS_Move(Output_Buffer_Addr, Block_Out_Addr, B, 0);
	if(Print_En){
...
int FPGA_Acc(void){
	...
	XDma_PS_Move((u32)(&Coeff[0]), Bram0_Base_Addr+(Const_Num+N+B)*4, N, 0);
	printf("Moving invariant data to FPGA time: %lld!\n\r", PS_Timer_Return());
	...
	for(it=0; it<L/B; it++){
		...
		PS_Timer_Start();
		// Move N+B data of Sig_In to FPGA
		Block_In_Addr = (u32)(&Sig_In_Ext[it*B]);
		Block_Out_Addr = (u32)(&FPGA_Sig_Out[it*B]);
		if(it==0){
...
void CPU_Compute(){
	...
	for(i=0; i<L; i++){
		...
		for(j=0; j<N; j++){
			if(i>=j){
				ARM_Sig_Out[i] += Sig_In[i-j] * Coeff[j];
			}
		}
	}

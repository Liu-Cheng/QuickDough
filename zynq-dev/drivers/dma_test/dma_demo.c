...
dma_demo_read(struct file *file, char __user *buf, size_t count, loff_t *ppos){
    // Add your own read interface if necessary
	return 0;
}

static ssize_t
dma_demo_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos){
    // Add your own write interface if necessary
	return 0;
}
...
static long dma_demo_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
    ...
    switch(cmd){
        ...
        case IOCTL_DMA_SET_PS_TO_PL:
            {
                iowrite32(dma_phys, dma_demo_ptr + DMA_DEMO_SRC_OFFSET);
                iowrite32(PL_DDR_ADDR, dma_demo_ptr + DMA_DEMO_DST_OFFSET);
                break;
            }
        case IOCTL_DMA_SET_PL_TO_PS:
            {
                iowrite32(PL_DDR_ADDR, dma_demo_ptr + DMA_DEMO_SRC_OFFSET);
                iowrite32(dma_phys, dma_demo_ptr + DMA_DEMO_DST_OFFSET);
                break;
            }
        case IOCTL_DMA_SET_LEN:
            {
                //printk("I am here now. \n");
                __get_user(len, (int *)arg);
                //printk("len = %d\n", len);
...
static int dma_demo_probe(struct platform_device *pdev){
    ...
    if ((dma_cls = class_create(THIS_MODULE, DEVICE_NAME)) == NULL){
        printk( KERN_ALERT "Class creation failed\n" );
        return -1;
    }

    if(device_create(dma_cls, NULL, devno, NULL, DEVICE_NAME) == NULL){
...
static struct of_device_id dma_demo_of_match[] = {
	{ .compatible = "xlnx,face-ip-top-1.0", },
	{ /* end of list */ },
};

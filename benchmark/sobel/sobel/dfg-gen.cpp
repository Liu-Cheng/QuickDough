...
void Head_File_Dump(int Gx[3][3], int Gy[3][3]){
    ...
    */
    ...
    fHandle << "char Block_Fig_In[" << B <<"][" << C << "];" << std::endl;
    fHandle << "int Gx[" << 3 << "][" << 3 <<"];" << std::endl;
    fHandle << "int Gy[" << 3 << "][" << 3 <<"];" << std::endl;
    fHandle << "char Block_Fig_Out[" << B << "][" << C << "];" << std::endl;
    
    fHandle << std::endl << "#endif" << std::endl << std::endl;
...
void Initial_IO_Placement(const std::string &DFG_Name){
    ...
    const int Kernel_Num_Per_Block = (B-2)/(R-2); //total number of the kernel iteration
...
void Loop_Stat(const std::vector<Operand*> &OP_Array, const std::vector<Instruction*> &Inst_Array){
    ...
    fHandle << "DFG_In_Num " << Const_Num + 3*3*2 + R*C/4 << std::endl;
    fHandle << "DFG_Out_Num " << R*C/4 << std::endl;
    fHandle << "DFG_Operation_Num " << Inst_Array.size() << std::endl;
    fHandle << "DFG_Total_Nodes " << OP_Array.size() << std::endl;
    fHandle << "Block_Repeat_Num " << B/R << std::endl;
    fHandle << "Block_Reuse_Num " << Const_Num + 2*C/4 + 3*3*2 << std::endl;
    fHandle << "Block_In_Num " << Const_Num + 3*3*2 + B*C/4 << std::endl;
    fHandle << "Block_Out_Num " << B*C/4 << std::endl;
    fHandle.close();

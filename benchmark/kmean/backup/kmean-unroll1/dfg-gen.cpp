...
void io_init(int sub_in[M*D+N*D+N*D+N], int sub_out[N*D+N]){
    ...
    int kernel_in[M*D+N*D+N*D+N]={0
//#include "kernel_in.txt"
    };

    int kernel_out[N*D+N]={0
//#include "kernel_out.txt"
    };
   
   for(int i=0; i<M*D+N*D+N*D+N; i++){
...
void kernel_to_dfg(std::vector<Operand*> &op_array, std::vector<Instruction*> &inst_array){
    ...
    for(int i=0; i<M; i++){
       ...
       int src0_op_id;
       //---------------------------------------------
       src0_op_id = data_to_id(D*M+N*D+8, INVAR);
       op_last[8] = branch_in_loop(op_array, inst_array, i, cond_op0->op_id, src0_op_id, data_to_id(1), 0, op_last[8], data_to_id(8, OUTVAR));

       src0_op_id = data_to_id(D*M+N*D, INVAR);
       op_last[0] = branch_in_loop(op_array, inst_array, i, cond_op0->op_id, src0_op_id, data_to_id(i*D, INVAR), 0, op_last[0], data_to_id(0, OUTVAR));

       src0_op_id = data_to_id(D*M+N*D+1, INVAR);
       op_last[1] = branch_in_loop(op_array, inst_array, i, cond_op0->op_id, src0_op_id, data_to_id(i*D+1, INVAR), 0, op_last[1], data_to_id(1, OUTVAR));

       //----------------------------------------------
       src0_op_id = data_to_id(D*M+N*D+9, INVAR);
       op_last[9] = branch_in_loop(op_array, inst_array, i, cond_op1->op_id, src0_op_id, data_to_id(1), 0, op_last[9], data_to_id(9, OUTVAR));

       src0_op_id = data_to_id(D*M+N*D+2, INVAR);
       op_last[2] = branch_in_loop(op_array, inst_array, i, cond_op1->op_id, src0_op_id, data_to_id(i*D, INVAR), 0, op_last[2], data_to_id(2, OUTVAR));

       src0_op_id = data_to_id(D*M+N*D+3, INVAR);
       op_last[3] = branch_in_loop(op_array, inst_array, i, cond_op1->op_id, src0_op_id, data_to_id(i*D+1, INVAR), 0, op_last[3], data_to_id(3, OUTVAR));

       //---------------------------------------------
       src0_op_id = data_to_id(M*D+N*D+10, INVAR);
       op_last[10] = branch_in_loop(op_array, inst_array, i, cond_op2->op_id, src0_op_id, data_to_id(1), 0, op_last[10], data_to_id(10, OUTVAR));

       src0_op_id = data_to_id(M*D+N*D+4, INVAR);
       op_last[4] = branch_in_loop(op_array, inst_array, i, cond_op2->op_id, src0_op_id, data_to_id(i*D, INVAR), 0, op_last[4], data_to_id(4, OUTVAR));

       src0_op_id = data_to_id(M*D+N*D+5, INVAR);
       op_last[5] = branch_in_loop(op_array, inst_array, i, cond_op2->op_id, src0_op_id, data_to_id(i*D+1, INVAR), 0, op_last[5], data_to_id(5, OUTVAR));

       //--------------------------------------------
       src0_op_id = data_to_id(D*M+N*D+11, INVAR);
       op_last[11] = branch_in_loop(op_array, inst_array, i, cond_op3->op_id, src0_op_id, data_to_id(1), 0, op_last[11], data_to_id(11, OUTVAR));

       src0_op_id = data_to_id(M*D+N*D+6, INVAR);
       op_last[6] = branch_in_loop(op_array, inst_array, i, cond_op3->op_id, src0_op_id, data_to_id(i*D, INVAR), 0, op_last[6], data_to_id(6, OUTVAR));

       src0_op_id = data_to_id(M*D+N*D+7, INVAR);
       op_last[7] = branch_in_loop(op_array, inst_array, i, cond_op3->op_id, src0_op_id, data_to_id(i*D+1, INVAR), 0, op_last[7], data_to_id(7, OUTVAR));
    }
}

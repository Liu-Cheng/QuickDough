...
void IO_Init(int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]);

void OP_Array_Init(std::vector<Operand*> &OP_Array, int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]);
...
void Head_File_Dump(int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]);

void IO_coe_Dump(int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]);
...
int main(){
    ...
    Head_File_Dump(Samples, Init_Centroids, Centroids_Sum, Centroids_Num);
    IO_coe_Dump(Samples, Init_Centroids, Centroids_Sum, Centroids_Num);
...
int Data_To_ID(std::string Name, int IDx, int IDy){
    ...
    if(Name=="Const_In"){
        ...
        return (IDx*D + IDy + S*D + Const_Num);
    }
    else if(Name=="Centroids_Sum"){
        return (IDx*D + IDy + S*D + N*D + Const_Num);
    }
    else if(Name=="Centroids_Num"){
        return (IDx + S*D +N*D + N*D + Const_Num);
    }
    else{
...
void IO_coe_Dump(int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]){
    ...
    for(int i=0; i<S; i++){
        for(int j=0; j<D; j++){
            Single_Dec_Dump(fHandle, (unsigned int) Samples[i][j], Width);
        }
    }
    ...
    for(int i=0; i<N; i++){
        for(int j=0; j<D; j++){
            Single_Dec_Dump(fHandle, (unsigned int)Centroids_Sum[i][j], Width);
        }
    }
    for(int i=0; i<N; i++){
        Single_Dec_Dump(fHandle, (unsigned int)Centroids_Num[i], Width);
    }
    fHandle.close();
...
void Head_File_Dump(int Samples[S][D], int Init_Centroids[N][D], int Centroids_Sum[N][D], int Centroids_Num[N]){
    ...
    fHandle << "int Samples[" << S << "][" << D <<"] = {" << std::endl;
    for(int i=0; i<S; i++){
        for(int j=0; j<D; j++){
            if(j==0){
                fHandle << "{" << Samples[i][j] << ", ";
    ...
    fHandle << "int Centroids_Sum[" << N << "][" << D <<"] = {" << std::endl;
    for(int i=0; i<N; i++){
        for(int j=0; j<D; j++){
            if(j==0){
                fHandle << "{" << Centroids_Sum[i][j] << ", ";
            }
            else if(i==N-1 && j==D-1){
                fHandle << Centroids_Sum[i][j] << "}" << std::endl;
            }
            else if(j==D-1){
                fHandle << Centroids_Sum[i][j] << "}, " << std::endl;
            }
    ...
    fHandle << "int Centroids_Num[" << N << "] = " << "{";
    for(int i=0; i<N; i++){
        if(i==N-1){
            fHandle << Centroids_Num[i] << "};" << std::endl << std::endl;
        }
        else{
            fHandle << Centroids_Num[i] << ", ";
        }
    }
...
void Initial_IO_Placement(const std::string &DFG_Name){
    ...
    int Const_In_Addr[1];
    int Samples_Addr[S][D];
    int Init_Centroids_Addr[N][D];
    int Centroids_Sum_Addr[N][D];
    int Centroids_Num_Addr[N];
    ...
    for(int i=0; i<S; i++){
        for(int j=0; j<D; j++){
            Samples_Addr[i][j] = Remapped_Bram0_Addr;
            Remapped_Bram0_Addr++ ;
        }
    }
    ...
    for(int i=0; i<N; i++){
        for(int j=0; j<D; j++){
            Centroids_Sum_Addr[i][j] = Remapped_Bram1_Addr;
            Remapped_Bram1_Addr++;
        }
    }

    for(int i=0; i<N; i++){
        Centroids_Num_Addr[i] = Remapped_Bram1_Addr;
        Remapped_Bram1_Addr++;
    }
    
    const int Work_Item_IO_Num = Const_Num + S*D + N*D + N*D + N; //total number of work-item's operand
    ...
    for(int p=0; p<Kernel_It_Num; p++){
        ...
        for(int i=0; i<S; i++){
            for(int j=0; j<D; j++){
                Kernel_IO_Addr[Row_Index][It] = Samples_Addr[i][j];
                if(p==0){
        ...
        for(int i=0; i<N; i++){
            for(int j=0; j<D; j++){
                Kernel_IO_Addr[Row_Index][It] = Centroids_Sum_Addr[i][j];
                if(p==0){
        ...
        for(int i=0; i<N; i++){
            Kernel_IO_Addr[Row_Index][It] = Centroids_Num_Addr[i];
            if(p==0){

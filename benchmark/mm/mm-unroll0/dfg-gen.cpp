...
void IO_coe_Dump(int A_In[N][N], int B_In[N][N], int C_Out[N][N]);
...
void Initial_IO_Placement(const std::string &DFG_Name){
    ...
    const int Work_Item_IO_Num = 1 + 3*N*N; // total number of work-item's operand
    ...
    for(int p=0; p<Kernel_It_Num; p++){
        ...
        for(int i=0; i<N; i++){
            for(int j=0; j<N; j++){
                Kernel_IO_Addr[Row_Index][It] = A_In_Addr[i][j];
                if(p==0){
        ...
        for(int i=0; i<N; i++){
            for(int j=0; j<N; j++){
                Kernel_IO_Addr[Row_Index][It] = C_Out_Addr[i][j];
                if(p==0){

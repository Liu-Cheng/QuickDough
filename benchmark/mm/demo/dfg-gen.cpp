...
//Here are actually M N-vector dot multiplication. In order to flatten the DFG, the vector
//dot multiplication is kind of parallelized. It performed N multiplication first and produces
//N results. Then every three results are further accumulated. After a few iterations, all the results
//are accumulated and the dot multiplication is done.
void Kernel_To_DFG(std::vector<Operand*> &OP_Array, std::vector<Instruction*> &Inst_Array){

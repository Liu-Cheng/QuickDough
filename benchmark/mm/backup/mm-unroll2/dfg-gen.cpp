...
void IO_coe_Dump(int Block_A_In[B], int Block_B_In[B], int Block_C_Out[B/M]);
...
void IO_coe_Dump(int Block_A_In[B], int Block_B_In[B], int Block_C_Out[B/M]){
    ...
    for(int i=0; i<B; i++){
        Single_Dec_Dump(fHandle, (unsigned int)Block_A_In[i], Width);
    }
...
void Head_File_Dump(){
   ...
   for(int i=0; i<B; i++){
    ...
    fHandle << "int Block_A_In[" << B << "] = {";
    for(int i=0; i<B; i++){
        if(i==B-1){
            fHandle << Block_A_In[i] << "}; " << std::endl << std::endl;
        }
        else{
            fHandle << Block_A_In[i] << ", ";
        }
    }

    fHandle << "int Block_B_In[" << B << "] = {";
    for(int i=0; i<B; i++){
        if(i==B-1){

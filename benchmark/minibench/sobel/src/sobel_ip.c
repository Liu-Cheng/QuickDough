...
void sobel_ip(int sub_in[R*C/4+18], int sub_out[R*C/4]){
	...
	for(i=1; i<R-1; i=i+1){
		//The reason that we set the kernel column rage to be [4, C-4] instead of [2, C-2]
		//is we will move all the input to main memory while we must handle 4 pixl at a time due
		//to the buffer data width. Limiting the kernel column range to be [4, C-4] will allow us to read
		//a word from buffer and the 4 pixl are right in the same word.

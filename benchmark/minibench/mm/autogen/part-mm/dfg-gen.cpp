...
void io_init(int sub_in[N][M+1], int sub_out[M]);
void op_array_init(std::vector<Operand*> &op_array, int sub_in[N][M+1], int sub_out[M], int const_in[1]);
...
void io_init(int sub_in[N][M+1], int sub_out[M]){
    ...
    for(int i=0; i<M; i++){
        sub_out[i]=0;
        for(int j=0; j<N; j++){
            sub_out[i]+=sub_in[j][0]*sub_in[j][i+1];
        }
    }
        ...
        for(int j=0; j<N; j++){
            ...
            if(j==0){
                ...
                inst_ptr->Set_Instruction( \
                        dst_ptr->op_id, \
                        MULADD, \
                        data_to_id(j, 0, INVAR), \
                        data_to_id(j, i+1, INVAR), \
                        data_to_id(0));

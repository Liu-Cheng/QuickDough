...
int Samples[S][D];
int Centroids_Sum[(S/M)*N][D];
int Centroids_Num[(S/M)*N];
...
void FPGA_Kernel(u32 Block_In_Addr, u32 Block_Centroids_Sum_Addr, u32 Block_Centroids_Num_Addr, int Print_En);
void Kmean_Kernel();
...
void FPGA_Kernel(u32 Block_In_Addr, u32 Block_Centroids_Sum_Addr, u32 Block_Centroids_Num_Addr, int Print_En){
	...
	XDma_PS_Move(Block_In_Addr, Input_Buffer_Addr, B*D, 0);
	...
	Xil_DCacheFlushRange(Block_Centroids_Sum_Addr, (B/M)*N*D*4);
	Output_Buffer_Addr = Bram1_Base_Addr;
	XDma_PS_Move(Output_Buffer_Addr, Block_Centroids_Sum_Addr, (B/M)*N*D, 0);

	Output_Buffer_Addr = Bram1_Base_Addr + (B/M)*N*D*4;
	Xil_DCacheFlushRange(Block_Centroids_Num_Addr, (B/M)*N*4);
	XDma_PS_Move(Output_Buffer_Addr, Block_Centroids_Num_Addr, (B/M)*N, 0);
...
int FPGA_Acc(void){
	...
	u32 Block_In_Addr;
	u32 Block_Centroids_Sum_Addr;
	u32 Block_Centroids_Num_Addr;
	...
	PS_Timer_Start();
	GPIO_Move(Const_In, Bram0_Base_Addr, Const_Num, 1);
	printf("Moving constant data to FPGA time: %lld!\n\r", PS_Timer_Return());

	int rit;
	int it;
	int Print_En;
	for(rit=0; rit<IT; rit++){

		PS_Timer_Start();
		GPIO_Move(&IM_Centroids[0][0], Bram0_Base_Addr + (Const_Num+B*D)*4, N*D, 1);
		if(rit==0){
			printf("Moving partial-invariant data to FPGA time: %lld!\n\r", PS_Timer_Return());
		}
		...
		for(it=0; it<S/B; it++){
			...
			// Move B nodes to FPGA
			Block_In_Addr = (u32)(&Samples[it*B][0]);
			Block_Centroids_Sum_Addr = (u32)(&Centroids_Sum[it*(B/M)*N][0]);
			Block_Centroids_Num_Addr = (u32)(&Centroids_Num[it*(B/M)*N]);

			if(it==0 && rit==0){
				printf("Input organization time: %lld!\n\r", PS_Timer_Return());
			}
			...
			FPGA_Kernel(Block_In_Addr, Block_Centroids_Sum_Addr, Block_Centroids_Num_Addr, Print_En);

			// Doing Nothing here
			PS_Timer_Start();
			if(it==0 && rit==0){
				printf("Output organization time: %lld!\n\r", PS_Timer_Return());
			}
		}
		...
		for(it=0; it<S/M; it++){
			for(i=0; i<N; i++){
				if(it==0){
					FPGA_Centroids_Num[i] = Centroids_Num[i];
				}
				else {
					FPGA_Centroids_Num[i] += Centroids_Num[it*N+i];
				}

				for(j=0; j<D; j++){
					if(it==0){
						FPGA_Centroids_Sum[i][j] = Centroids_Sum[i][j];
					}
					else{
						FPGA_Centroids_Sum[i][j] += Centroids_Sum[it*N+i][j];
					}
				}
			}
		}
		...
		if(rit==0){
			printf("Division on ARM time: %lld!\n\r", PS_Timer_Return());
		}
	}
...
void Kmean_Kernel(){
	...
	for(it=0; it<IT; it++){
		...
		for(i=0; i<S; i++){
			dist0=(Samples[i][0]-IM_Centroids[0][0])*(Samples[i][0]-IM_Centroids[0][0])+ \
					(Samples[i][1]-IM_Centroids[0][1])*(Samples[i][1]-IM_Centroids[0][1]);

			dist1=(Samples[i][0]-IM_Centroids[1][0])*(Samples[i][0]-IM_Centroids[1][0])+ \
					(Samples[i][1]-IM_Centroids[1][1])*(Samples[i][1]-IM_Centroids[1][1]);

			dist2=(Samples[i][0]-IM_Centroids[2][0])*(Samples[i][0]-IM_Centroids[2][0])+ \
					(Samples[i][1]-IM_Centroids[2][1])*(Samples[i][1]-IM_Centroids[2][1]);

			dist3=(Samples[i][0]-IM_Centroids[3][0])*(Samples[i][0]-IM_Centroids[3][0])+ \
					(Samples[i][1]-IM_Centroids[3][1])*(Samples[i][1]-IM_Centroids[3][1]);

			if(dist0<=dist1 && dist0<=dist2 && dist0<=dist3){
				ARM_Centroids_Num[0] += 1;
				ARM_Centroids_Sum[0][0] += Samples[i][0];
				ARM_Centroids_Sum[0][1] += Samples[i][1];
			}

			if(dist1<dist0 && dist1<=dist2 && dist1<=dist3){
				ARM_Centroids_Num[1] += 1;
				ARM_Centroids_Sum[1][0] += Samples[i][0];
				ARM_Centroids_Sum[1][1] += Samples[i][1];
			}

			if(dist2<dist0 && dist2<dist1 && dist2<=dist3){
				ARM_Centroids_Num[2] += 1;
				ARM_Centroids_Sum[2][0] += Samples[i][0];
				ARM_Centroids_Sum[2][1] += Samples[i][1];
			}

			if(dist3<dist0 && dist3<dist1 && dist3<dist2){
				ARM_Centroids_Num[3] += 1;
				ARM_Centroids_Sum[3][0] += Samples[i][0];
				ARM_Centroids_Sum[3][1] += Samples[i][1];
			}
		}
...
void Search_Nearest_Nodes(int Avg_Centroids[N][D], int Calculated_Centroids[N][D]){
    ...
    for(i=0; i<N; i++){
        ...
        int dist;
        int nid=0;
        int min=(Avg_Centroids[i][0]-Samples[0][0])*(Avg_Centroids[i][0]-Samples[0][0]);
        min+=(Avg_Centroids[i][1]-Samples[0][1])*(Avg_Centroids[i][1]-Samples[0][1]);

        for(j=1; j<S; j++){
            dist=(Avg_Centroids[i][0]-Samples[j][0])*(Avg_Centroids[i][0]-Samples[j][0]);
            dist+=(Avg_Centroids[i][1]-Samples[j][1])*(Avg_Centroids[i][1]-Samples[j][1]);
            if(min>dist){
                nid=j;
                min=dist;
            }
        ...
        Calculated_Centroids[i][0]=Samples[nid][0];
        Calculated_Centroids[i][1]=Samples[nid][1];
    }

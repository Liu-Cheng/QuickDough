...
void Scheduler::Scheduling(){
    ...
    cout<<"Scheduling kernel starts!"<<endl;
...
void Scheduler::ListSchedulingAlgorithmPEFirst(){
    ...
    while(!scheduling_completed){
        ...
        //cout<<"ready list size="<<op_ready_set.size()<<endl;
        //cout<<"PE="<<selected_PE_id<<" OP="<<selected_op_id<<" scheduling times="<<scheduling_index<<endl;
...
int Scheduler::FetchOP(const int &src_op_id, const int &target_PE_id, const ExecutionMode &mode){
    int fetch_cost=0;
    int attached_PE_id;
    if(DFG->DFG_vertex[src_op_id]->vertex_attribute.vertex_state==DataAvail){
    ...
    for(int i=0; i<GLvar::maximum_simulation_time; i++){
        ...
        if(load_path_avail && write_port1_avail && read_port3_avail && read_port4_avail && read_port5_avail){
            if(mode==Implementation){
                ...
                CGRA->PE_array[in_PE_id]->component_trace[i+1]->component_activity->load_mux=1;
                ...
                //Dump the trace
                if(GLvar::report_level==1){
                    fTrace<<"Load "<<operation_id<<" through "<<in_PE_id<<" at time "<<i+2<<endl;
                }
            }
            load_cost=i+2;
            break;
...
void Scheduler::FromDSTToOutMem(const int &operation_id, const int &start_time){
    ...
    while(true){
        ...
        if(write0_avail && read0_avail){
            ...
            CGRA->PE_array[GLvar::store_PE_id]->component_trace[op_avail_time+1]->component_activity->store_op=operation_id;
            break;
        }
        else{
            op_avail_time++;
    ...
    if(last_op_store_time<op_avail_time){
        last_op_store_time=op_avail_time;
    }
}
...
int Scheduler::OperationTransmission(const int &start_time, const int &src_operation_id, const list<int> &routing_path, const ExecutionMode &mode){
    ...
    //Since IO load bings additional pipeline and mux, it complexs the transmission.
    ...
    for(int i=0; i<PE_num_on_path;){
        ...
        if(current_PE_id==GLvar::load_PE_id){
        ...
        if(i<PE_num_on_path-1){
            ...
            if(next_PE_id==GLvar::load_PE_id){
        ...
        if(PE_num_on_path==1){
            ...
            if(i==0){
               ...
               bool current_PE_output_avail=CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+4]->component_reserved->PE_output_reserved[child_id]==false;
               ...
               if(current_PE_memory_read_avail && current_PE_output_avail && next_PE_input_avail && next_PE_memory_write_avail && next_load_path_avail){
                   if(mode==Implementation){
                       ...
                       CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+4]->component_reserved->PE_output_reserved[child_id]=true;
                       CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+1]->component_activity->memory_wr_ena[1]=0;

                       if(GLvar::report_level==1){
                           fTrace<<"Move operation "<<src_operation_id<<" First: from "<<" PE "<<current_PE_id<<" to "<<" PE "<<next_PE_id<<" at time "<<transmission_progress_time<<endl;
                       }
                ...
                /*cout<<"XXX"<<endl;
                cout<<"PE_id="<<current_PE_id<<endl;
                cout<<"time="<<transmission_progress_time<<endl;
                cout<<"child_id="<<child_id<<endl;
                cout<<CGRA->PE_array.size()<<endl;
                cout<<CGRA->PE_array[current_PE_id]->component_trace.size()<<endl;
                cout<<CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+1]->component_reserved->PE_output_reserved.size()<<endl;*/
                ...
                bool next_PE_input_avail=CGRA->PE_array[next_PE_id]->component_trace[transmission_progress_time+3]->component_reserved->PE_input_reserved==false;
                ...
                if(current_PE_bypass_avail && current_PE_output_avail && next_PE_input_avail && next_PE_memory_write_avail && next_load_path_avail){
                    if(mode==Implementation){
                        ...
                        if(GLvar::report_level==1){
                            fTrace<<"Move operation "<<src_operation_id<<" bypass: from"<<" PE "<<current_PE_id<<" to "<<" PE "<<next_PE_id<<" at time "<<transmission_progress_time<<endl;
                        }

                        //int tmp_op=CGRA->PE_array[1]->component_trace[670]->component_activity->memory_port_op[0];
                        //if(tmp_op==1221){
                            //cout<<"checkpoint 3: right!"<<endl;
                        //}
                        //else{
                            //cout<<"checkpoint 3: wrong! tmp_op="<<tmp_op<<endl;
                        //}
                ...
                //Store and forward occasion
                    ...
                    if(mode==Implementation){
                        ...
                        if(current_PE_id==GLvar::load_PE_id){
                            ...
                            CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+2]->component_activity->load_mux=0;
                        }
                        ...
                        //if(src_operation_id==0 && current_PE_id==1){
                            //cout<<"cp2: attach_time="<<attach_point.attached_time<<endl;
                        //}
                    ...
                    while(true){
                        //int tmp_op=CGRA->PE_array[1]->component_trace[10]->component_activity->memory_port_op[0];
                        //if(tmp_op==73){
                            //cout<<"checkpoint 4: right!"<<endl;
                        //}
                        //else{
                            //cout<<"checkpoint 4: wrong! tmp_op="<<tmp_op<<endl;
                        //}
                        //cout<<"current_PE_id="<<current_PE_id<<endl;
                        //cout<<"next_PE_id="<<next_PE_id<<endl;
                        //cout<<"current_pipeline="<<current_additional_pipeline<<endl;
                        //cout<<"write0_reserve="<<CGRA->PE_array[1]->component_trace[670]->component_reserved->memory_write_reserved[0]<<endl;
                        ...
                        //cout<<"write0_reserve="<<current_PE_memory_write_avail0<<endl;

                        bool current_PE_memory_read_avail=(current_PE_memory_read_avail0 || current_PE_memory_read_avail1 || current_PE_memory_read_avail2) && current_PE_memory_write_avail0;
                        bool current_PE_output_avail=CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+6+current_additional_pipeline]->component_reserved->PE_output_reserved[child_id]==false;
                        bool next_PE_input_avail=CGRA->PE_array[next_PE_id]->component_trace[transmission_progress_time+6+current_additional_pipeline]->component_reserved->PE_input_reserved==false;

                        bool next_load_path_avail=true;
                        if(next_PE_id==GLvar::load_PE_id){
                            next_load_path_avail=CGRA->PE_array[next_PE_id]->component_trace[transmission_progress_time+7+current_additional_pipeline+next_additional_pipeline]->component_reserved->load_path_reserved==false;
                        }
                        ...
                        if(current_PE_memory_read_avail && current_PE_output_avail && next_PE_input_avail && next_PE_memory_write_avail && next_load_path_avail){
                            if(mode==Implementation){
                                ...
                                CGRA->PE_array[current_PE_id]->component_trace[transmission_progress_time+6+current_additional_pipeline]->component_reserved->PE_output_reserved[child_id]=true;
                                ...
                                //tmp_op=CGRA->PE_array[1]->component_trace[10]->component_activity->memory_port_op[0];
                                //if(tmp_op==73){
                                    //cout<<"checkpoint 5: right!"<<endl;
                                //}
                                //else{
                                    //cout<<"checkpoint 5: wrong! tmp_op="<<tmp_op<<endl;
                                //}
                                //cout<<"current_PE_id="<<current_PE_id<<endl;
                                //cout<<"next_PE_id="<<next_PE_id<<endl;
                                //cout<<"current_pipeline="<<current_additional_pipeline<<endl;
                                //cout<<"write0_reserve="<<CGRA->PE_array[1]->component_trace[10]->component_reserved->memory_write_reserved[0]<<endl;
...
int Scheduler::OperationExecution(const int &target_operation_id, const vector<int> &src_operation_ids, const int &target_PE_id, const vector<int> &arrival_time, const ExecutionMode &mode){
    ...
    if(mode==Implementation){
        if(GLvar::report_level==1){
            fTrace<<"Operation "<<target_operation_id<<" is executed at "<<" PE "<<target_PE_id<<" at time "<<execution_time<<endl;
        }
        ...
        TargetPERefresh(src_operation_ids, target_operation_id, start_time, target_PE_id);
            ...
            //}
            //else{
                //cout<<"checkpoint 4, wrong! op="<<tmp_op<<endl;
            //}

        TargetOperationRefresh(src_operation_ids, target_operation_id, target_PE_id, execution_time);

            //tmp_op=CGRA->PE_array[1]->component_trace[670]->component_activity->memory_port_op[0];
            //if(tmp_op==1221){
                //cout<<"checkpoint 5, right!"<<endl;
            //}
            //else{
                //cout<<"checkpoint 5, wrong! op="<<tmp_op<<endl;
            //}
...
void Scheduler::TargetPERefresh(const vector<int> &src_operation_ids, const int &target_operation_id, const int &start_time, const int &target_PE_id){
    ...
    //if(target_operation_id==1221){
        //cout<<"I have reserved the PE "<<target_PE_id<<" write port at "<<start_time+7<<endl;
    //}
    ...
    //int tmp_op=CGRA->PE_array[1]->component_trace[670]->component_activity->memory_port_op[0];
    //if(tmp_op==1221){
        //cout<<"checkpoint 0, right!"<<endl;
    //}
    //else{
        //cout<<"checkpoint 0, wrong! op="<<tmp_op<<endl;
    //}
...
void Scheduler::DataMemoryAnalysis(){
    ...
    for(int i=0; i<GLvar::CGRA_scale; i++){
        ...
        //Refresh the die time & birth time in memory write port
        for(int j=0; j<GLvar::maximum_simulation_time; j++){
            for(int p=0; p<2; p++){
                ...
                if(wr_reserve){
                    if(birth_time[wr_op]==NaN){
                        birth_time[wr_op]=j;
                    }
                    else if(birth_time[wr_op]>j){
                        birth_time[wr_op]=j;
                    }

                    //When it is output, the die time is probably one cycle larger than birth time if
                    //there is no reference in this graph.
                    //bool is_output=DFG->DFG_vertex[wr_op]->vertex_type==OutputData;
                    //if(is_output){
                        //die_time[wr_op]=birth_time[wr_op]+1;
                    //}
                }
            }
            ...
            for(int p=0; p<6; p++){
                ...
                if(rd_reserve){
                    ...
                    else if(die_time[rd_op]<j){
                        die_time[rd_op]=j;
                    }

                    //bool is_input=DFG->DFG_vertex[rd_op]->vertex_type==InputData;
                    ...
                    //}
                    //else{
                        //bool attachedToCurrPE=DFG->DFG_vertex[rd_op]->vertex_attribute.execution_PE_id==i;
                        //if(is_input && attachedToCurrPE){
                            //if(birth_time[rd_op]==NaN){
                                //birth_time[rd_op]=j-1;
                            //}
                            //else if(birth_time[rd_op]>(j-1)){
                                //birth_time[rd_op]=j-1;
                            //}
                        //}
                    //}
        ...
        //Analyze the maximum data memory
        for(int j=0; j<GLvar::maximum_operation_num; j++){
            if(birth_time[j]==NaN && die_time[j]>0){
                cout<<"op exception is "<<j<<" , it has "<<DFG->DFG_vertex[j]->children.size()<<"children and "<<DFG->DFG_vertex[j]->parents.size()<<"parents!";
...
void Scheduler::AddrGen(const vector<int> &birth_time, const vector<int> &die_time, const int &memCapacity, const int &PE_id){
    //Allocate an address to an operand when it gets to being in data memory.
    //When some of the input or constant data existed at the begining, we may accordingly
    //allocate at the begining.
    //Also note that there are two occasions that requires address allocation
    //1) Initial data of the data memory
    //2) The data that is writen into data memory
    //Only when the data is last referenced, the address will be released.
    list<int> addrAvail;
    map<int, int> opToAddr;

    for(int i=1; i<memCapacity+10; i++){
        addrAvail.push_front(i);
    }
...
void Scheduler::InstructionDumpMif(int finalExecutionTime){
    for(int i=0; i<GLvar::CGRA_scale; i++){
        ostringstream os;
        ...
        if(!fHandle.is_open()){
            DEBUG1("Fail to create the PE.mif");
        }
        list<int> bitList;
        ...
        int width;
        int inst_delay=0;
        for(int j=0; j<finalExecutionTime;){
            //Make sure the CGRA does nothing but to wait for the delay of the instruction memory
            if(inst_delay<GLvar::inst_memory_delay){
                inst_delay++;
            }
            else{
                j++;
            ...
            //pop operation for load fifo or push operation for store fifo
            if(i==GLvar::load_PE_id){
                if(CGRA->PE_array[i]->component_trace[j]->component_activity->load_mux==0){
                    fHandle<<1;
                }
                else{
                    fHandle<<0;
            ...
            else if(i==GLvar::store_PE_id){
                if(CGRA->PE_array[i]->component_trace[j]->component_activity->store_op!=NaN){
                    fHandle<<1;
                }
                else{
                    fHandle<<0;
            ...
            //mmeory ena
            for(int l=0; l<2; l++){
                fHandle<<CGRA->PE_array[i]->component_trace[j]->component_activity->memory_wr_ena[l];
            }

            //memory addr
            for(int l=0; l<6; l++){
                dec_data=CGRA->PE_array[i]->component_trace[j]->component_activity->memory_addr[l];
                if(dec_data==NaN){
                    dec_data=0;
                }
                width=GLvar::maximum_data_memory_addr_width;
                while(dec_data!=1 && dec_data!=0){
                    bitList.push_back(dec_data%2);
                    dec_data=dec_data/2;
                    width--;
...
void Scheduler::InstructionDumpMem(int finalExecutionTime){
    ...
    for(int i=0; i<GLvar::CGRA_scale; i++){
        ...
        fHandle.open(fName.c_str());
        if(!fHandle.is_open()){
            DEBUG1("Fail to open the PE.mif");
        }

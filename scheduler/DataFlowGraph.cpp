...
void DataFlowGraph::DFGConstruct(){
    if(GLvar::report_level==4){
        MatrixMultiplyInstGen(MATRIX_M,MATRIX_N,MATRIX_P);
    }
    if(GLvar::report_level==200){
        RandomInstGen();
    }
    DFGGen();
    DFGScaleRefresh();
...
void DataFlowGraph::DFGGen(){
    ...
    cout<<"Load "<<DFG_init_file<<endl;

    ifstream DFG_init_handle;
    DFG_init_handle.open(DFG_init_file.c_str());
    if(!DFG_init_handle.is_open()){
        DEBUG1("DFG init file open error!");
    }
    map<int, int> op_to_value;
    while(!DFG_init_handle.eof()){
        int tmp1;
        int tmp2;
        DFG_init_handle >> tmp1; //operation id
        DFG_init_handle >> tmp2; //operation value
        if(DFG_init_handle.fail()){
            break;
        }
        if(op_to_value.count(tmp1)>0){
            DEBUG1("Repeated DFG initialization!");
        }
        op_to_value[tmp1] = tmp2;
    }
    DFG_init_handle.close();
    ...
    while(!DFG_inst_handle.eof()){
        ...
        for(int i=0; i<5; i++){
            ...
            //The field indicates instruction type instead of operation.
            if(i==1){
                continue;
            }
            if(max_vertex_id<tmp){
                max_vertex_id=tmp;
            }
        }
    ...
    //DFG_inst_handle.clear();
    //DFG_inst_handle.seekg(0, ios::beg);
    int edge_id=0;
    int inst_id=0;
    while(!DFG_inst_handle.eof()){
        ...
        //Since each destination of an instruction must be a new vertex,
        //instance a new vertex directly.
...
void DataFlowGraph::DFGScaleRefresh(){
    vertex_num=0;
    input_vertex_num=0;
    output_vertex_num=0;
    inter_vertex_num=0;
...
void DataFlowGraph::MatrixMultiplyInstGen(int m, int n, int p){
    string fName="./config/MatrixMultiply.s";
    string fNameA="./config/MatrixA.data";
    string fNameB="./config/MatrixB.data";
    string MM_init_file="./config/MatrixMultiply.init";

    ofstream fHandle;
    ofstream fHandleA;
    ofstream fHandleB;
    ofstream MM_init_handle;

    fHandle.open(fName.c_str());
    if(!fHandle.is_open()){
        DEBUG1("Failed to create the MatrixMultiply.s!");
    }

    MM_init_handle.open(MM_init_file.c_str());
    if(!MM_init_handle.is_open()){
        DEBUG1("Failed to create MatrixMultiply.init");
    }
    MM_init_handle<< 0 << " "<< 0 <<endl;
    ...
    fHandleA.open(fNameA.c_str());
    if(!fHandleA.is_open()){
        DEBUG1("Failed to create the MatrixA.data!");
    }
    for(int i=0; i<m; i++){
        for(int j=0; j<n; j++){
            int tmp=rand()%10+1;
            fHandleA<<tmp<<" ";
            MM_init_handle<<srcMatrix1[i][j]<<" "<<tmp<<endl;
        }
        fHandleA<<"\n";
    }
    fHandleA.close();

    fHandleB.open(fNameB.c_str());
    if(!fHandleB.is_open()){
        DEBUG1("Failed to create the MatrixB.data!");
    }
    for(int i=0; i<n; i++){
        for(int j=0; j<p; j++){
            int tmp=rand()%10+1;
            fHandleB<<tmp<<" ";
            MM_init_handle<<srcMatrix2[i][j]<<" "<<tmp<<endl;
        }
        fHandleB<<"\n";
    }
    fHandleB.close();
    ...
    for(int i=0; i<m; i++){
        for(int j=0; j<p; j++){
            for(int k=0; k<n; k++){
                if(k==0){
                    fHandle<<OPID<<" ";
                    fHandle<<GetOpcodeValue("Mul")<<" ";
                    fHandle<<srcMatrix1[i][k]<<" ";
                    fHandle<<srcMatrix2[k][j]<<" ";
                    fHandle<<"0"<<endl;
                    OPID++;
                }
                else if(k==n-1){
                    fHandle<<dstMatrix[i][j]<<" ";
                    fHandle<<GetOpcodeValue("MulAdd")<<" ";
                    fHandle<<srcMatrix1[i][k]<<" ";
                    fHandle<<srcMatrix2[k][j]<<" ";
                    fHandle<<(OPID-1)<<endl;
                }
